...
const PEOPLE = ["ali", "ahmad", "shahid", "devi", "usman"];

// authorOf[i] picks who wrote SITES[i]; two authors (ali, devi) get
...
async function seed() {
  ...
  const dead = ["nowhere.zz", "wreckindex.zz"];
  ...
  PEOPLE.forEach((name, pIdx) => {
    ...
    // 'usman' has been nearly everywhere; others get shorter trails.
    const stops = name === "usman" ? 18 : 6 + Math.floor(rand() * 6);
  ...
  console.log(
    `Seeded ${SITES.length} sites, ${PEOPLE.length} people, ${visitDocs.length} visits.`,
  );
